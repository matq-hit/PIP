...
train_mode = 'SDI_MG'
train_mode_dict = {
    'fully_supervised': 'SegmentationClassAug',
    'WSSL_CRF': 'SegmentationClassBboxCRF',
    'SDI_MG': 'VOC12_M&G+',
}
...
def train():
    ...
    train_loader = torch.utils.data.DataLoader(
        VOCDataset(split='train_aug', label_dir_path=train_mode_dict[train_mode], crop_size=321, is_scale=True, is_flip=True),
...
def test(model_path_test, use_crf):
    ...
    val_loader = torch.utils.data.DataLoader(
        VOCDataset(split='val', crop_size=crop_size,
                   label_dir_path='SegmentationClassAug', is_scale=False, is_flip=False),
        batch_size=batch_size,
        shuffle=False,
        num_workers=4,
        drop_last=False
    ...
    for iter_id, batch in enumerate(val_loader):
        ...
        labels = losses.resize_labels(
            labels, size=(crop_size, crop_size)).to(device)
        logits = model(images)
        ...
        for i in range(len(image_ids)):
            if is_post_process:
                raw_image = cv2.imread(
                    img_dir_path + image_ids[i] + '.jpg', cv2.IMREAD_COLOR)  # shape = [H, W, 3]
                ...
                raw_image = cv2.copyMakeBorder(
                    raw_image, value=[0, 0, 0], **pad_kwargs)
                raw_image = raw_image.astype(np.uint8)
                start_time = time.time()
                prob = post_processor(
                    raw_image, probs[i].detach().cpu().numpy())
            ...
            accuracy = float(torch.eq(outputs[i], labels[i]).sum(
            ).cpu()) / (logits.shape[2] * logits.shape[3])
            index += 1
            if index % 200 == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--model_path_test', default='./exp/model_last_20000.pth', help='test model path')
